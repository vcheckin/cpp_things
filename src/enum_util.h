 ...
 */
template <typename A>
struct is_bitmap: public std::false_type{};

/** bit of synt sugar for expressions like
 *  id == ID1 || id == ID2 || id == ID3
 *  oneOf(id, {ID1, ID2, ID3});
 */
template <typename /* @TODO `typename` -> `std::equality_comparable` when <concepts> hdr is available*/ T> 
...
constexpr
typename std::enable_if<is_bitmap<Enum>::value, bool>::type
isSet(const Enum& a) 
{
